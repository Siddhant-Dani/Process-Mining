...
with open('output_test_V2.txt', 'w') as f:
    sys.stdout = f
    # Parameters
    ...
    def prepare_features(probabilities):
        ...
        std_prob = np.std(probabilities, axis=1)  # Adding standard deviation as an additional feature
        return np.column_stack([mean_prob, var_prob, std_prob])
    ...
    def train_classifier(features, labels):
        clf = RandomForestClassifier(max_depth=10, min_samples_split=10, min_samples_leaf=4, n_estimators=100)
        skf = StratifiedKFold(n_splits=5)  # Implementing k-fold cross-validation
        cv_scores = cross_val_score(clf, features, labels, cv=skf, scoring='f1')  # Evaluating with F1 score
        print(f'Cross-Validated F1 Scores: {cv_scores}, Mean F1 Score: {np.mean(cv_scores)}')
        clf.fit(features, labels)  # Training the classifier on the full data after validation
        return clf
        ...
        def encode_and_pad(log):
            ...
            for trace in log:
                ...
                if isinstance(encoded_trace, list):
                    padded_trace = pad_sequences([encoded_trace], maxlen=max_sequence_length, padding='pre')
    ...
    def compute_accuracy_rates_and_fscore(test_similarities, base_similarities):
        ...
        confusion = confusion_matrix([1] * len(test_similarities) + [0] * len(base_similarities),
                                     [1 if sim >= 0.5 else 0 for sim in test_similarities] +
                                     [1 if sim >= 0.5 else 0 for sim in base_similarities])
        return P, R, F, confusion
    ...
    def main(models_directory, test_logs_directory, base_logs_directory, mapping_files_directory,
        ...
        model_files = sort_numerically([f for f in os.listdir(models_directory) if f.endswith('.h5')])
        ...
        mapping_files = sort_numerically([f for f in os.listdir(mapping_files_directory) if f.endswith('.txt')])
        probability_files = [f for f in os.listdir(probabilities_files_directory) if f.endswith('.npy')]

        # Check for equal number of files
        if not (len(model_files) == len(test_log_files) == len(base_log_files) == len(mapping_files) == len(
                probability_files)):
            raise ValueError(
                "The number of model files, test logs, base logs, mapping files, and probability files must be equal.")
        ...
        for i in range(num_logs):
            ...
            for j, (test_similarity, base_similarity) in enumerate(zip(test_log_similarities, base_log_similarities)):
                print(
                    f"Trace {j + 1}: Test Similarity = {test_similarity:.4f}, Base Similarity = {base_similarity:.4f}")

            P, R, F, confusion = compute_accuracy_rates_and_fscore(test_log_similarities, base_log_similarities)
            total_precision += P
            total_recall += R
            total_fscore += F
        ...
        average_precision = total_precision / num_logs
        average_recall = total_recall / num_logs
    ...
    if __name__ == "__main__":
        ...
        test_logs_directory = "D:/Siddhant/Masters Project/Dataset/Process Discovery Contest 2023_1_all/Test Logs 4"  # keep changing this one
        base_logs_directory = "D:/Siddhant/Masters Project/Dataset/Process Discovery Contest 2023_1_all/Base Logs 4"  # Keep changing this one
        ...
        main(models_directory, test_logs_directory, base_logs_directory, mapping_files_directory,
             probabilities_files_directory, max_sequence_length)
